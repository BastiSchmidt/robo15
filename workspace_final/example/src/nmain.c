...
int sensor()
{
	...
	for (i = 0; i<3;i++)
	{
		systick_wait_ms(20);
		display_clear(1);
		printnumber(ecrobot_get_sonar_sensor(NXT_PORT_S4),3,3);
		if (ecrobot_get_sonar_sensor(NXT_PORT_S4) < x)
		{
			x = ecrobot_get_sonar_sensor(NXT_PORT_S4);
			ecrobot_sound_tone(100, 300, 50);
			display_clear(1);
			printnumber(42,1,1);
...
int scan()
{
	...
	if (distance < 40 && distance!=0 )
		{
			ultrasonic_detect = (orientation + 1) % 4;
		}
	drehen_grad_r(180);
	...
	if (distance < 40 && distance!=0 )
		{
			ultrasonic_detect = (orientation + 3) % 4;
		};
	int intersection = 0;
	switch(orientation)
	{
...
void turn_d(int direction){
	...
	float korrektur = 0.94736842;
	float korrektur2 = 0.642857;
	int Wert90 = (90+2*360)*korrektur;
	int Wert180 = (180+2*360)*korrektur;
	int Wert902 = 90*korrektur2;
	int Wert1802 = 180*korrektur2;
	int power= -100;
	switch (diff){
	case 0:
				break;
			case 1:
				...
				while (abs(nxt_motor_get_count(NXT_PORT_A))<Wert90)
				{
					nxt_motor_set_speed(NXT_PORT_A, power , 0);
				}
				...
				nxt_motor_set_count(NXT_PORT_A, 0);
				while (abs(nxt_motor_get_count(NXT_PORT_A))<Wert902)
				{
					nxt_motor_set_speed(NXT_PORT_A, -power , 0);
				}
				nxt_motor_set_speed(NXT_PORT_A, 0 , 1);
			...
			case 2:
				nxt_motor_set_count(NXT_PORT_A, 0);
				while (abs(nxt_motor_get_count(NXT_PORT_A))<Wert180)
				{
					nxt_motor_set_speed(NXT_PORT_A, -power , 0);
				}
				nxt_motor_set_speed(NXT_PORT_A, 0 , 1);
				...
				nxt_motor_set_count(NXT_PORT_A, 0);
				while (abs(nxt_motor_get_count(NXT_PORT_A))<Wert1802)
				{
					nxt_motor_set_speed(NXT_PORT_A, power , 0);
				}
				nxt_motor_set_speed(NXT_PORT_A, 0 , 1);
...
struct coord bfs_closest_unvisited_node() {
	...
	do {
		...
		for (i = 0; i <= 3; i++) {
			neighbour_position = shift_coordinates(position,(i+ultrasonic_detect) % 4);

			if ((ptrmap[position.x + MAZE_WIDTH][position.y +
					MAZE_HEIGHT])->compass[(i+ultrasonic_detect) % 4] != NULL) {
				if (ptrmap[position.x + MAZE_WIDTH][position.y + MAZE_HEIGHT]
						->compass[(i+ultrasonic_detect) % 4]->visited == 0) {
					wanted = shift_coordinates(position, (i+ultrasonic_detect) % 4);
					destroy_list(queue);
					destroy_list(seen);
					ultrasonic_detect=0;
